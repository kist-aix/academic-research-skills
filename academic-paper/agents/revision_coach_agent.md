...
3. For each identified phrase, emit one `commitment` object:
   ...
   - `required_evidence_type`: What the revised manuscript must contain to fulfill this — `new_section` / `new_figure` / `new_table` / `new_citation` / `methods_paragraph` / `discussion_paragraph` / `acknowledgment_only`. This guides the `re_review_mode_protocol` verification step in Schema 11 v3.11.
